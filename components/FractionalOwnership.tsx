  ...
  const placeTradingOrder = async () => {
    try {
      ...
      const orderData = {
        ...
        amount: tradeAmount, // Keep as normal number, not wei
        price: tradePrice,   // Keep as normal number, not wei
        isBuyOrder,
        expiration,
  ...
  const addLiquidity = async () => {
    try {
      ...
      const liquidityData = {
        userId: user?.id,
        shareToken: selectedToken.address,
        shareAmount: liquidityShareAmount, // Keep as normal number
        usdAmount: liquidityUsdAmount      // Keep as normal number
      };
